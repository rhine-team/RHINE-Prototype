...
var log = clog.NewWithPlugin("rhine")

type (
	// Rhine is the plugin that reads zone data from disk.
	Rhine struct {
		Next plugin.Handler
		Zones
...
func (rh Rhine) ServeDNS(ctx context.Context, w dns.ResponseWriter, r *dns.Msg) (int, error) {
	...
	zone := plugin.Zones(rh.Zones.Names).Matches(qname)
	if zone == "" {
		return plugin.NextOrFailure(rh.Name(), rh.Next, ctx, w, r)
	}

	z, ok := rh.Zones.Z[zone]
	if !ok || z == nil {
...
func (rh Rhine) Name() string { return "rhine" }
